function [pval, table, warningCode] = wwtest(varargin)
%WWTEST Parametric Watson-Williams multi-sample test for equal means. Can be used as a one-way ANOVA test for circular data.
%
% [pval, table] = circ.wwtest(alpha, idx, [w])
% [pval, table, warningCode] = circ.wwtest(alpha1, alpha2, [w1, w2])
...
%     warningCode  Integer, with higher values encoding greater warning
%                   about bad assumptions being made. Value of 0 means
%                   we passed the assumption check without issue.
%
% PHB 3/19/2009
%
% References:
...
warningCode = checkAssumption(rw,mean(pn),verbose);
...
function warningCode = checkAssumption(rw,n,verbose)
warningCode = 0;
if n >= 11 && rw<.45
    if verbose
